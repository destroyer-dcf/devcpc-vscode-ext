...
exports.toContainerFile = exports.hexToPRG = exports.hexToAmsDosBin = exports.hexToKCC = void 0;
// see: https://en.wikipedia.org/wiki/Intel_HEX
const types_1 = require("./types");
function findMinMaxAddr(lines) {
    let minAddr = 0xFFFF;
    let maxAddr = 0;
    lines.forEach((line) => {
        if (line.startsWith(':')) {
...
function appendHexData(arr, minAddr, headerSize, lines) {
    lines.forEach((line) => {
        if (line.startsWith(':')) {
            const numBytes = parseInt(line.slice(1, 3), 16);
...
function hexToKCC(hex, execAddr, withExecAddr) {
...
exports.hexToKCC = hexToKCC;
function hexToAmsDosBin(hex, execAddr) {
    const lines = hex.split('\n');
    const { minAddr, maxAddr } = findMinMaxAddr(lines);
...
exports.hexToAmsDosBin = hexToAmsDosBin;
function hexToPRG(hex) {
    const lines = hex.split('\n');
    const { minAddr, maxAddr } = findMinMaxAddr(lines);
...
exports.hexToPRG = hexToPRG;
// wrap an emulator specific data blob into a container file
function toContainerFile(data, type, startAddr, start, stopOnEntry) {
    const hdrLength = 16;
...
exports.toContainerFile = toContainerFile;
