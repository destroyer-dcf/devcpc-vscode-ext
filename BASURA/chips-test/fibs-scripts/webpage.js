...
exports.addWebpageCommand = void 0;
// fibs command to build and serve the emulators webpage
// deno-lint-ignore-file no-unversioned-import
const fibs_1 = require("jsr:@floooh/fibs");
const colors_1 = require("jsr:@std/fmt/colors");
function addWebpageCommand(c) {
    c.addCommand({ name: "webpage", help, run });
}
exports.addWebpageCommand = addWebpageCommand;
function help() {
    fibs_1.log.helpCmd([
        "webpage build",
        "webpage serve",
...
function run(p, args) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (subcmd === "build") {
            if (fibs_1.util.dirExists(dstDir)) {
                if (Deno.env.get('CI') || fibs_1.log.ask(`Ok to delete directory ${dstDir}?`, false)) {
                    Deno.removeSync(dstDir, { recursive: true });
                }
            }
            fibs_1.util.ensureDir(dstDir);
            yield fibs_1.proj.generate(config);
            yield fibs_1.proj.build({});
            yield deployWebpage(p, srcDir, dstDir);
            fibs_1.log.info(`\n${(0, colors_1.green)("Done.")} (webpage available in ${dstDir})`);
        }
...
function deployWebpage(p, srcDir, dstDir) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        for (const item of items) {
            ...
            const { title, system, url, img, note, } = item;
            const uiUrl = url.replaceAll('.html', '-ui.html');
            fibs_1.log.info(`> adding thumbnail for ${url}`);
            content += `<div class="thumb-frame">\n`;
            content += `  <div class="thumb-title">${title}</div>\n`;
            if (fibs_1.util.fileExists(`${srcDir}/${system}-ui.js`)) {
        ...
        for (const file of ['style.css', 'favicon.png', 'help.html']) {
            fibs_1.log.info(`> copy file: ${file}`);
        ...
        for (const assetDir of assetDirs) {
            ...
            if (!fibs_1.util.dirExists(srcAssetDir)) {
                fibs_1.log.warn(`source asset directory ${srcAssetDir} not found!`);
                continue;
            }
            fibs_1.util.ensureDir(dstAssetDir);
            for (const file of Deno.readDirSync(srcAssetDir)) {
                if (!file.isFile) {
                    fibs_1.log.warn(`ignoring non-file asset ${file.name} in ${srcAssetDir}`);
                    continue;
                }
                ...
                if (srcPath.endsWith('.jpg')) {
                    ...
                    yield cwebp(srcPath, dstPath);
                }
                else {
                    fibs_1.log.info(`> copy ${srcPath} => ${dstPath}`);
                    Deno.copyFileSync(srcPath, dstPath);
                }
            }
        }
...
function toWebpExt(path) {
    return path.replace(/\.jpg$/, '.webp');
}
function cwebp(srcPath, dstPath) {
    return __awaiter(this, void 0, void 0, function* () {
        fibs_1.log.info(`> cwebp ${srcPath} => ${dstPath}`);
        yield fibs_1.util.runCmd('cwebp', {
