...
#if defined(__EMSCRIPTEN__)
// Nueva función para insertar discos DSK directamente desde JavaScript
EMSCRIPTEN_KEEPALIVE bool webapi_insert_disc(void* ptr, int size) {
    if (!ptr || size <= 0) {
        return false;
    }
    chips_range_t dsk_data = { .ptr = ptr, .size = (size_t)size };
    return cpc_insert_disc(&state.cpc, dsk_data);
}
#endif
